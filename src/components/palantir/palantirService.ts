export const saveToPalantir = async <T>(
    ...
    options: {
      objectType: string;
      ontologyRidEnvVar: string;
    }
  ): Promise<void> => {
    const { objectType, ontologyRidEnvVar } = options;
    
    try {
      console.log(`Saving ${data.length} ${objectType.toLowerCase()}s to Palantir...`);
      
      const FOUNDRY_URL = process.env.NEXT_PUBLIC_FOUNDRY_URL;
      const ONTOLOGY_RID = process.env[ontologyRidEnvVar];
      
      if (!FOUNDRY_URL || !ONTOLOGY_RID) {
        throw new Error(`Missing required environment variables: ${!FOUNDRY_URL ? 'FOUNDRY_URL ' : ''}${!ONTOLOGY_RID ? ontologyRidEnvVar + ' ' : ''}`);
      }
  ...
  export const savePublicationsToPalantir = async (
    ...
    return saveToPalantir(publications, accessToken, {
      ...
      ontologyRidEnvVar: 'NEXT_PUBLIC_PUBLICATION_ONTOLOGY_RID'
    });
  };
  ...
  export const saveScholarsToPalantir = async (
    ...
    return saveToPalantir(scholars, accessToken, {
      objectType: 'Scholar',
      ontologyRidEnvVar: 'NEXT_PUBLIC_ONTOLOGY_RID'
    });
  };
